...
const fromEmail = process.env.FROM_EMAIL || 'noreply@send.danloski.com';
...
export async function POST(request) {
  try {
    const { to, subject, templateData } = await request.json();
    ...
    const html = getMarketingTemplate(templateData);
    ...
    const { data, error } = await resend.emails.send({
      ...
      subject: subject,
      html: html,
      reply_to: templateData.restaurant_email || fromEmail,
    });
